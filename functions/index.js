...
exports.trackHelperRequestStats = onDocumentWritten('serviceRequests/{requestId}', async (event) => {
  ...
  if (afterStatus === SERVICE_REQUEST_STATUS.HELPER_FOUND && afterOfferHelperId) {
    ...
    if (didChangeOffer) {
      await createUserNotification({
        userId: afterOfferHelperId,
        title: 'New helper request',
        message: `A ${after.topic || after.subject || 'service'} request is waiting for your response.`,
        type: 'helper_offer',
        requestId,
...
function getCustomerSafeAiErrorMessage(error) {
  const message = getSafeErrorMessage(error, 'Unable to process customer AI turn.');
...
function getAiSecrets() {
  ...
  const secrets = {
    ...
    location: groupedSecrets.GOOGLE_CLOUD_LOCATION || groupedSecrets.VERTEX_AI_LOCATION || groupedSecrets.FIREBASE_AI_LOCATION || '',
    GOOGLE_CLOUD_LOCATION: groupedSecrets.GOOGLE_CLOUD_LOCATION || '',
    VERTEX_AI_LOCATION: groupedSecrets.VERTEX_AI_LOCATION || '',
    FIREBASE_AI_LOCATION: groupedSecrets.FIREBASE_AI_LOCATION || '',
...
exports.customerServiceAiTurn = onRequest(
  {
    cors: true,
    secrets: [UNCEDO_AI_KEYS],
    timeoutSeconds: 120,
  ...
  async (req, res) => {
    ...
    try {
      ...
      logger.error('customer_service_ai_turn_missing_ai_config', {
        uid: decoded.uid,
        error: error.message,
      });
      res.status(500).json({
    ...
    const serviceCatalog = Array.isArray(body.serviceCatalog) ? body.serviceCatalog : [];
    const questionPlan = body.questionPlan || {};
    const conversation = Array.isArray(body.conversation) ? body.conversation : [];
    const customerText = String(body.customerText || '').trim();
    const appInstruction = String(body.appInstruction || '').trim();

    logger.info('customer_service_ai_turn_started', {
      uid: decoded.uid,
      isGreeting: !customerText && !appInstruction,
      customerName,
      hasCustomerText: Boolean(customerText),
      hasAppInstruction: Boolean(appInstruction),
      conversationCount: conversation.length,
    });

    try {
      const result = await generateCustomerServiceAiTurn({
        firebaseConfig: aiConfig,
        customerName,
        requestState,
        serviceCatalog,
      ...
      logger.info('customer_service_ai_turn_completed', {
        uid: decoded.uid,
        speakLength: String(result.speak || '').trim().length,
        status: String(result.status || ''),
        usagePromptTokens: Number(result?.usageSummary?.promptTokenCount || 0),
        usageResponseTokens: Number(result?.usageSummary?.responseTokenCount || 0),
      });
      ...
      const safeMessage = getCustomerSafeAiErrorMessage(error);
      logger.error('customer_service_ai_turn_failed', {
        uid: decoded.uid,
        error: error.message,
        safeMessage,
      });
...
exports.getHelperAgreement = onRequest({ cors: true }, async (req, res) => {
  if (req.method !== 'GET') {
    res.status(405).json({ success: false, message: 'Method not allowed' });
    return;
  }
  ...
  await ensureHelperAgreementSeeded({ db, admin }).catch((error) => {
    logger.warn('Unable to seed helper agreement before reading.', { error: error.message });
  });

  const userSnap = await db.collection('users').doc(decoded.uid).get();
  const userData = userSnap.data() || {};
  const isHelper = String(userData.activeRole || userData.role || '').toLowerCase() === 'helper';
  if (!isHelper && !isAdminToken(decoded)) {
    res.status(403).json({ success: false, message: 'Only helpers can access the Helper Agreement.' });
    return;
  }

  const bundle = await getHelperAgreementBundle({ db, admin, userId: decoded.uid });
...
exports.acceptHelperAgreement = onRequest({ cors: true, memory: '1GiB' }, async (req, res) => {
  if (req.method !== 'POST') {
    res.status(405).json({ success: false, message: 'Method not allowed' });
    return;
  }
  ...
  const isHelper = String(userData.activeRole || userData.role || '').toLowerCase() === 'helper';
  if (!isHelper) {
    res.status(403).json({ success: false, message: 'Only helpers can accept the Helper Agreement.' });
    return;
  }

  const { ipAddress, userAgent } = getRequestMetadata(req);
  try {
    const acceptanceResult = await acceptHelperAgreement({
      db,
      admin,
      user: { uid: decoded.uid, ...userData },
      typedSignatureName: req.body?.typedSignatureName,
    ...
    const safeMessage = getSafeErrorMessage(error, 'Unable to accept Helper Agreement.');
    logger.warn('Helper agreement acceptance failed.', {
      uid: decoded.uid,
      message: safeMessage,
      errorType: typeof error,
    });
...
exports.publishHelperAgreementVersion = onRequest({ cors: true }, async (req, res) => {
  if (req.method !== 'POST') {
    res.status(405).json({ success: false, message: 'Method not allowed' });
    return;
  }
  ...
  try {
    const result = await publishHelperAgreementVersion({
      db,
      admin,
      version,
      title: String(req.body?.title || HELPER_AGREEMENT_TITLE).trim() || HELPER_AGREEMENT_TITLE,
      effectiveDate: String(req.body?.effectiveDate || '').trim(),
      contentMarkdown: String(req.body?.contentMarkdown || '').trim() || buildHelperAgreementMarkdown(),
      changeSummary: String(req.body?.changeSummary || '').trim(),
      reviewedBy: String(req.body?.reviewedBy || 'Uncedo').trim() || 'Uncedo',
      reviewedAt: String(req.body?.reviewedAt || '').trim(),
      nextReviewAt: String(req.body?.nextReviewAt || '').trim(),
      stampLabel: String(req.body?.stampLabel || '').trim(),
      updatedBy: decoded.email || decoded.uid,
      status: String(req.body?.status || HELPER_AGREEMENT_STATUS.ACTIVE).trim().toLowerCase() === HELPER_AGREEMENT_STATUS.DRAFT
    ...
    logger.warn('Failed to publish helper agreement version.', {
      uid: decoded.uid,
      message: error.message,
    });
    res.status(400).json({ success: false, message: error.message || 'Unable to publish version.' });
...
exports.getNearbyActiveCustomersMapData = onRequest({ cors: true }, async (req, res) => {
  if (req.method !== 'POST') {
    res.status(405).json({ success: false, message: 'Method not allowed' });
    return;
  }
  ...
  const radiusKm = Math.min(
    MAX_NEARBY_HELPERS_RADIUS_KM,
    Math.max(1, normalizePositiveNumber(req.body?.radiusKm, MAX_NEARBY_HELPERS_RADIUS_KM)),
  );
  const limit = Math.min(
    MAX_NEARBY_HELPERS_LIMIT,
    Math.max(1, Math.floor(normalizePositiveNumber(req.body?.limit, 24))),
  ...
  const helperRef = db.collection('users').doc(decoded.uid);
  ...
  try {
    ...
    const customers = customersSnap.docs
      .map((item) => ({ uid: item.id, ...item.data() }))
      .filter(isCustomerMapVisible)
      .map((customer) => {
        const location = getLastKnownLocationSnapshot(customer);
        const distanceKm = computeDistanceKm(origin, location);
        const fullName = String(customer.fullName || customer.displayName || customer.firstName || 'Customer').trim();

        return {
          id: customer.uid,
          fullName,
          initials: getInitials(fullName),
          profilePhoto: String(customer.profilePhoto || customer.selfieUrl || customer.photoURL || '').trim(),
          distanceKm,
          lastKnownLocation: location,
        };
      })
      .filter((customer) => Number.isFinite(customer.distanceKm) && customer.distanceKm <= radiusKm)
      .sort((a, b) => a.distanceKm - b.distanceKm)
      .slice(0, limit)
      .map((customer) => ({
        id: customer.id,
        ...
        distanceKm: Number(customer.distanceKm.toFixed(2)),
        lastKnownLocation: {
          latitude: customer.lastKnownLocation.latitude,
          longitude: customer.lastKnownLocation.longitude,
          updatedAtMs: customer.lastKnownLocation.updatedAtMs,
        },
      }));
    ...
    res.status(500).json({
      success: false,
      message: getSafeErrorMessage(error, 'Unable to load nearby customers right now.'),
    });
  }
});
